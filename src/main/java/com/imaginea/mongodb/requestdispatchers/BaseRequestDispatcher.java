...
public class BaseRequestDispatcher {
     ...
     */
    protected static String validateDbInfo(String dbInfo, Logger logger,
            HttpServletRequest request) {

        String response = null;
        if (dbInfo == null) {
            InvalidHTTPRequestException e = new InvalidHTTPRequestException(
                    ErrorCodes.DB_INFO_ABSENT, "Token Id not provided");
            return formErrorResponse(logger, e);
        }
        // Check if db information is present in session
        HttpSession session = request.getSession();
        @SuppressWarnings("unchecked")
        List<String> mongosInSession = (List<String>) session
                .getAttribute("dbInfo");

        InvalidHTTPRequestException e = null;
        if (mongosInSession == null) {
            e = new InvalidHTTPRequestException(ErrorCodes.INVALID_SESSION,
                    "No Mongo Config parameters present in Session.");
            return formErrorResponse(logger, e);
        }
        if (!mongosInSession.contains(dbInfo)) {
            e = new InvalidHTTPRequestException(ErrorCodes.INVALID_SESSION,
                    "Provided Mongo Config parameters not present in session");
            return formErrorResponse(logger, e);
        }
        return response;
    }
     ...
     */
    protected static String formErrorResponse(Logger logger,
    ...
    protected static class ResponseTemplate {
        public String execute(Logger logger, String dbInfo,
                HttpServletRequest request, ResponseCallback callback) {
            ...
            try {
                ...
                ApplicationException e = new ApplicationException(
                        ErrorCodes.ERROR_PARSING_PORT, "Invalid Port",
                        m.getCause());
                response = formErrorResponse(logger, e);
            } catch (IllegalArgumentException m) {
                // When port out of range
                ApplicationException e = new ApplicationException(
                        ErrorCodes.PORT_OUT_OF_RANGE, "Port out of range",
                        m.getCause());
                response = formErrorResponse(logger, e);
                ...
                ApplicationException e = new ApplicationException(
                        ErrorCodes.ANY_OTHER_EXCEPTION, m.getMessage(),
                        m.getCause());
                response = formErrorResponse(logger, e);
            }
