...
/**
 * Defines resources for performing create/delete/update operations on documents present inside collections in databases
 * in Mongo we are currently connected to. Also provide resources to get list of all documents present inside a
 ...
 * GET and POST request resources for documents are defined here. For PUT and DELETE functionality , a POST request with
...
public class DocumentRequestDispatcher extends BaseRequestDispatcher {
    ...
    /**
     * Maps GET Request to get list of documents inside a collection inside a database present in mongo db to a service
     * function that returns the list. Also forms the JSON response for this request and sent it to client. In case of
     * any exception from the service files an error object if formed.
     * 
    ...
    public String getQueriedDocsList(@PathParam("dbName") String dbName, @PathParam("collectionName") String collectionName, @QueryParam("query") String query,
            @QueryParam("tokenId") String tokenId, @QueryParam("fields") String fields, @QueryParam("limit") String limit, @QueryParam("skip") String skip,
    ...
    /**
     * Maps GET Request to get all keys of document inside a collection inside a database present in mongo db to a
     * service function that returns the list. Also forms the JSON response for this request and sent it to client. In
    ...
    @Path("/keys")
    @Produces(MediaType.APPLICATION_JSON)
    public String getKeysRequest(@PathParam("dbName") String dbName, @PathParam("collectionName") String collectionName, @QueryParam("tokenId") String tokenId,
    ...
    /**
     * Maps POST Request to perform operations like update/delete/insert document inside a collection inside a database
     * present in mongo db to a service function that returns the list. Also forms the JSON response for this request
     * and sent it to client. In case of any exception from the service files an error object if formed.
     * 
     * @param dbName Name of Database
     * @param collectionName Name of Collection
    ...
    public String postDocsRequest(@PathParam("dbName") String dbName, @PathParam("collectionName") String collectionName,
            @DefaultValue("POST") @QueryParam("action") String action, @FormParam("document") String documentData, @FormParam("_id") String _id, @FormParam("keys") String keys,
        ...
        try {
            ...
            response = formErrorResponse(logger, e.getMessage(), ErrorCodes.INVALID_OBJECT_ID, e.getStackTrace(), "ERROR");
        } catch (JSONException e) {
            response = "{\"code\":" + "\"" + ErrorCodes.JSON_EXCEPTION + "\"," + "\"message\": \"Error while forming JSON Object\"}";
        } catch (DatabaseException e) {
            response = formErrorResponse(logger, e.getMessage(), e.getErrorCode(), e.getStackTrace(), "ERROR");
